...
export function useLiveAPI(systemInstruction: string, onLockInTriggered?: () => void) {
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          onopen: async () => {
            ...
            // Jumpstart the agent immediately after connection
            setTimeout(() => {
              if (sessionRef.current !== sessionPromise) return; // double check if still valid
              sessionPromise.then((session: any) => {
                 try {
                   const startMessage = { turns: [{ role: 'user', parts: [{ text: "Hello! The user has connected. Please begin the interaction according to your system instructions." }] }] };
                   if (typeof session.sendClientContent === 'function') {
                     session.sendClientContent({ turns: startMessage.turns, turnComplete: true });
                   } else if (typeof session.send === 'function') {
                     session.send({ clientContent: { turns: startMessage.turns, turnComplete: true } });
                   }
                 } catch(e) {
            ...
            silenceIntervalRef.current = setInterval(() => {
              if (Date.now() - lastActivityTimeRef.current > 90000) {
                 sessionPromise.then((session: any) => {
                    try {
                      const silenceMessage = { turns: [{ role: 'user', parts: [{ text: "SYSTEM STATUS: The user has been silent for 90 seconds. Say exactly: 'I'm here ready to help when you are, or you can say end call or bye, to end this call.'" }] }] };
                      if (typeof session.sendClientContent === 'function') {
                        session.sendClientContent({ turns: silenceMessage.turns, turnComplete: true });
                      } else if (typeof session.send === 'function') {
                        session.send({ clientContent: { turns: silenceMessage.turns, turnComplete: true } });
                      }
                      lastActivityTimeRef.current = Date.now();
                    } catch(e) {}
                 });
          ...
          onmessage: (message: LiveServerMessage) => {
               ...
               message.serverContent.modelTurn.parts.forEach((part: any) => {
                  ...
                  if (part.inlineData && part.inlineData.data) {
                     try {
                        ...
                        console.error("Audio playback queue error:", e);
                     }
                  }
               });
            }
            ...
            if (message.goAway) {
              console.log("GoAway signal received, session limit reached. Disconnecting gracefully.");
  ...
  const setupAudioCapture = async (sessionPromise: any) => {
    ...
    try {
      ...
      // RACE CONDITION FIX: If the component unmounted or timed out while waiting for the mic...
      if (!sessionRef.current) {
        stream.getTracks().forEach(t => t.stop());
        return;
      }
      
      // Stop any "ghost" streams if they survived
      ...
      processor.onaudioprocess = (e) => {
        ...
        if (sessionRef.current && base64Data.length > 0) {
          sessionRef.current.then((session: any) => {
             try {
  ...
  const playNextAudio = () => {
    ...
    try {
      ...
      console.error("Audio buffer playback error:", e);
      isPlayingRef.current = false;
      playNextAudio();
    }
  };
